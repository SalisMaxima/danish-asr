...
| Log output mailed, not written to file | `$USER` not expanded in `#BSUB -o/-e` | Use LSF `%U` placeholder in `#BSUB` log path directives — LSF does not expand shell variables in `#BSUB` headers, only in the script body |
