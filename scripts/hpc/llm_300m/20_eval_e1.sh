#!/bin/bash
#BSUB -J danish_asr_eval_llm_300m_e1
#BSUB -q gpua100
#BSUB -n 4
...
#BSUB -o /work3/s204696/logs/lsf/eval_llm_300m_e1_%J.out
#BSUB -e /work3/s204696/logs/lsf/eval_llm_300m_e1_%J.err
#
...
#
# Optional overrides:
#   RUN_DIR=/work3/$USER/outputs/omniasr_llm_300m_e1 \
#       bsub < scripts/hpc/llm_300m/20_eval_e1.sh
#   MODEL_PATH=/work3/$USER/outputs/omniasr_llm_300m_e1/ws_1.<hash>/checkpoints/step_20000/model \
#       bsub < scripts/hpc/llm_300m/20_eval_e1.sh
#   EVAL_CONFIG=configs/fairseq2/llm_300m/llm-eval-e1-read-aloud.yaml \
#       bsub < scripts/hpc/llm_300m/20_eval_e1.sh
...
RUN_DIR="${RUN_DIR:-/work3/$USER/outputs/omniasr_llm_300m_e1}"
EXPECTED_STEP="${EXPECTED_STEP:-step_20000}"
EVAL_OUT_DIR="${EVAL_OUT_DIR:-/work3/$USER/outputs/omniasr_llm_300m_e1_eval}"
...
resolve_model_path() {
    ...
    if [ -z "$ws_dir" ] || [ ! -d "$ws_dir" ]; then
        ...
        echo "ERROR: Set WORKSPACE_DIR=/work3/\$USER/outputs/omniasr_llm_300m_e1/ws_1.<hash> if needed." >&2
        exit 1
    fi
...
render_config() {
    ...
    local model_path="$2"
    local rendered
    rendered="$(mktemp /tmp/llm_eval_300m_XXXXXX.yaml)"
    sed "s|__MODEL_PATH__|$model_path|g" "$template" > "$rendered"
    echo "$rendered"
}

echo "=== LLM 300M V2 E1 Evaluation (old fairseq2 path) ==="
...
CONFIGS=(
    "configs/fairseq2/llm_300m/llm-eval-e1.yaml"
    "configs/fairseq2/llm_300m/llm-eval-e1-read-aloud.yaml"
    "configs/fairseq2/llm_300m/llm-eval-e1-conversation.yaml"
)

for i in "${!CONFIGS[@]}"; do
    CONFIG_TEMPLATE="${EVAL_CONFIG:-${CONFIGS[$i]}}"
    CONFIG_RENDERED="$(render_config "$CONFIG_TEMPLATE" "$MODEL_PATH")"
    TAGS="llm_300m_v2,e1,eval,test,$(split_tag "$CONFIG_TEMPLATE")"
